//===-- GameBoyISelLowering.cpp - GameBoy DAG Lowering Implementation -------------===//
//
...
GameBoyTargetLowering::GameBoyTargetLowering(const GameBoyTargetMachine &TM,
  ...
  // Do not use MUL. The GameBoy instructions are closer to SMUL_LOHI &co.
...
const char *GameBoyTargetLowering::getTargetNodeName(unsigned Opcode) const {
#define NODE(name)                                                             \
  case GameBoyISD::name:                                                           \
    return #name
...
EVT GameBoyTargetLowering::getSetCCResultType(const DataLayout &DL, LLVMContext &,
                                          EVT VT) const {
  assert(!VT.isVector() && "No GameBoy SetCC type for vectors!");
  return MVT::i8;
}
...
SDValue GameBoyTargetLowering::LowerDivRem(SDValue Op, SelectionDAG &DAG) const {
...
SDValue GameBoyTargetLowering::LowerGlobalAddress(SDValue Op,
  ...
  // This code could be used as a replacement for folding in the constant offset.
  // For greater transparency, a pseudo instruction for register offset will be created
  // so that it is clear throughout the backend when this behaviour is expected.
...
SDValue GameBoyTargetLowering::LowerBlockAddress(SDValue Op,
                                             SelectionDAG &DAG) const {
  auto DL = DAG.getDataLayout();
...
/// IntCCToGameBoyCC - Convert a DAG integer condition code to an GameBoy CC.
static GameBoyCC::CondCodes intCCToGameBoyCC(ISD::CondCode CC) {
  switch (CC) {
  default:
...
SDValue GameBoyTargetLowering::getGameBoyCmp(SDValue LHS, SDValue RHS,
  ...
  if (LHS.getSimpleValueType() == MVT::i16 && isa<ConstantSDNode>(RHS)) {
    ...
    SDValue RHShi = DAG.getNode(ISD::EXTRACT_ELEMENT, DL, MVT::i8, RHS,
                                DAG.getIntPtrConstant(1, DL));
    Cmp = DAG.getNode(GameBoyISD::CMP, DL, MVT::Glue, LHSlo, RHSlo);
    Cmp = DAG.getNode(GameBoyISD::CMPC, DL, MVT::Glue, LHShi, RHShi, Cmp);
  } else {
...
/// Returns appropriate GameBoy CMP/CMPC nodes and corresponding condition code for
/// the given operands.
SDValue GameBoyTargetLowering::getGameBoyCmp(SDValue LHS, SDValue RHS, ISD::CondCode CC,
                                     SDValue &GameBoycc, SelectionDAG &DAG,
                                     SDLoc DL) const {
  SDValue Cmp;
  ...
  case ISD::SETGT: {
    if (const ConstantSDNode *C = dyn_cast<ConstantSDNode>(RHS)) {
      ...
      case -1: {
        ...
        UseTest = true;
        GameBoycc = DAG.getConstant(GameBoyCC::COND_PL, DL, MVT::i8);
        break;
      }
      case 0: {
  ...
  case ISD::SETLT: {
    if (const ConstantSDNode *C = dyn_cast<ConstantSDNode>(RHS)) {
      ...
      case 0: {
        ...
        UseTest = true;
        GameBoycc = DAG.getConstant(GameBoyCC::COND_MI, DL, MVT::i8);
        break;
      }
      }
    }
  ...
  if (VT == MVT::i32) {
    ...
    if (UseTest) {
      ...
      SDValue Top = DAG.getNode(ISD::EXTRACT_ELEMENT, DL, MVT::i8, LHShi,
                                DAG.getIntPtrConstant(1, DL));
      Cmp = DAG.getNode(GameBoyISD::TST, DL, MVT::Glue, Top);
    } else {
      Cmp = getGameBoyCmp(LHSlo, RHSlo, DAG, DL);
      Cmp = DAG.getNode(GameBoyISD::CMPC, DL, MVT::Glue, LHShi, RHShi, Cmp);
    }
  } else if (VT == MVT::i64) {
    ...
    if (UseTest) {
      ...
      Cmp = getGameBoyCmp(LHS0, RHS0, DAG, DL);
      Cmp = DAG.getNode(GameBoyISD::CMPC, DL, MVT::Glue, LHS1, RHS1, Cmp);
      Cmp = DAG.getNode(GameBoyISD::CMPC, DL, MVT::Glue, LHS2, RHS2, Cmp);
      Cmp = DAG.getNode(GameBoyISD::CMPC, DL, MVT::Glue, LHS3, RHS3, Cmp);
    }
  } else if (VT == MVT::i8 || VT == MVT::i16) {
    if (UseTest) {
      // When using tst we only care about the highest part.
      Cmp = DAG.getNode(GameBoyISD::TST, DL, MVT::Glue,
                        (VT == MVT::i8)
                            ? LHS
                            : DAG.getNode(ISD::EXTRACT_ELEMENT, DL, MVT::i8,
                                          LHS, DAG.getIntPtrConstant(1, DL)));
    } else {
      Cmp = getGameBoyCmp(LHS, RHS, DAG, DL);
    }
  } else {
    llvm_unreachable("Invalid comparison size");
  }

  // When using a test instruction GameBoycc is already set.
  if (!UseTest) {
    GameBoycc = DAG.getConstant(intCCToGameBoyCC(CC), DL, MVT::i8);
  }

  return Cmp;
}

SDValue GameBoyTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue TargetCC;
  SDValue Cmp = getGameBoyCmp(LHS, RHS, CC, TargetCC, DAG, dl);

  return DAG.getNode(GameBoyISD::BRCOND, dl, MVT::Other, Chain, Dest, TargetCC,
                     Cmp);
}

SDValue GameBoyTargetLowering::LowerSELECT_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(GameBoyISD::SELECT_CC, dl, VTs, Ops);
}

SDValue GameBoyTargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  SDValue LHS = Op.getOperand(0);
  SDValue RHS = Op.getOperand(1);
...
SDValue GameBoyTargetLowering::LowerVASTART(SDValue Op, SelectionDAG &DAG) const {
...
void GameBoyTargetLowering::ReplaceNodeResults(SDNode *N,
...
bool GameBoyTargetLowering::isLegalAddressingMode(const DataLayout &DL,
  ...
  if (isa<PointerType>(Ty) && AS == GameBoy::ProgramMemory) {
    return false;
  }
...
bool GameBoyTargetLowering::getPreIndexedAddressParts(SDNode *N, SDValue &Base,
                                                  SDValue &Offset,
  ...
  if (const LoadSDNode *LD = dyn_cast<LoadSDNode>(N)) {
    ...
    if (GameBoy::isProgramMemoryAccess(ST)) {
      return false;
    }
  } else {
    return false;
...
bool GameBoyTargetLowering::getPostIndexedAddressParts(SDNode *N, SDNode *Op,
                                                   SDValue &Base,
                                                   SDValue &Offset,
                                                   ISD::MemIndexedMode &AM,
                                                   SelectionDAG &DAG) const {
...
bool GameBoyTargetLowering::isOffsetFoldingLegal(
...
///
SDValue GameBoyTargetLowering::LowerCallResult(
...
MachineBasicBlock *GameBoyTargetLowering::insertShift(MachineInstr &MI,
  ...
  Register ShiftAmtReg = RI.createVirtualRegister(&GameBoy::GPR8RegClass);
  Register ShiftAmtReg2 = RI.createVirtualRegister(&GameBoy::GPR8RegClass);
  ...
  BuildMI(CheckBB, dl, TII.get(GameBoy::PHI), ShiftReg)
      .addReg(SrcReg)
      .addMBB(BB)
      .addReg(ShiftReg2)
      .addMBB(LoopBB);
  BuildMI(CheckBB, dl, TII.get(GameBoy::PHI), ShiftAmtReg)
      .addReg(ShiftAmtSrcReg)
      .addMBB(BB)
      .addReg(ShiftAmtReg2)
      .addMBB(LoopBB);
  BuildMI(CheckBB, dl, TII.get(GameBoy::PHI), DstReg)
      .addReg(SrcReg)
      .addMBB(BB)
      .addReg(ShiftReg2)
      .addMBB(LoopBB);
...
static bool isCopyMulResult(MachineBasicBlock::iterator const &I) {
  if (I->getOpcode() == GameBoy::COPY) {
    Register SrcReg = I->getOperand(1).getReg();
    return (SrcReg == GameBoy::R0 || SrcReg == GameBoy::R1);
  }
...
MachineBasicBlock *
GameBoyTargetLowering::insertCopyR1(MachineInstr &MI, MachineBasicBlock *BB) const {
  const TargetInstrInfo &TII = *Subtarget.getInstrInfo();
  MachineBasicBlock::iterator I(MI);
  BuildMI(*BB, I, MI.getDebugLoc(), TII.get(GameBoy::COPY))
      .add(MI.getOperand(0))
      .addReg(GameBoy::R1);
  MI.eraseFromParent();
  return BB;
...
MachineBasicBlock *GameBoyTargetLowering::insertAtomicArithmeticOp(
  ...
  unsigned StoreOpcode = (Width == 8) ? GameBoy::STPtrRr : GameBoy::STWPtrRr;

  // Disable interrupts.
  BuildMI(*BB, I, dl, TII.get(GameBoy::INRdA), SCRATCH_REGISTER)
      .addImm(Subtarget.getIORegSREG());
  BuildMI(*BB, I, dl, TII.get(GameBoy::BCLRs)).addImm(7);
  ...
  // Restore interrupts.
  BuildMI(*BB, I, dl, TII.get(GameBoy::OUTARr))
      .addImm(Subtarget.getIORegSREG())
...
GameBoyTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
  ...
  case GameBoy::MULRdRr:
  case GameBoy::MULSRdRr:
    return insertMul(MI, MBB);
  case GameBoy::CopyR1:
    return insertCopyR1(MI, MBB);
  case GameBoy::AtomicLoadAdd8:
    return insertAtomicArithmeticOp(MI, MBB, GameBoy::ADDRdRr, 8);
  case GameBoy::AtomicLoadAdd16:
    return insertAtomicArithmeticOp(MI, MBB, GameBoy::ADDWRdRr, 16);
  case GameBoy::AtomicLoadSub8:
    return insertAtomicArithmeticOp(MI, MBB, GameBoy::SUBRdRr, 8);
  case GameBoy::AtomicLoadSub16:
    return insertAtomicArithmeticOp(MI, MBB, GameBoy::SUBWRdRr, 16);
  case GameBoy::AtomicLoadAnd8:
    return insertAtomicArithmeticOp(MI, MBB, GameBoy::ANDRdRr, 8);
  case GameBoy::AtomicLoadAnd16:
    return insertAtomicArithmeticOp(MI, MBB, GameBoy::ANDWRdRr, 16);
  case GameBoy::AtomicLoadOr8:
    return insertAtomicArithmeticOp(MI, MBB, GameBoy::ORRdRr, 8);
  case GameBoy::AtomicLoadOr16:
    return insertAtomicArithmeticOp(MI, MBB, GameBoy::ORWRdRr, 16);
  case GameBoy::AtomicLoadXor8:
    return insertAtomicArithmeticOp(MI, MBB, GameBoy::EORRdRr, 8);
  case GameBoy::AtomicLoadXor16:
    return insertAtomicArithmeticOp(MI, MBB, GameBoy::EORWRdRr, 16);
  }
  ...
  */
  ...
  BuildMI(*trueMBB, trueMBB->begin(), dl, TII.get(GameBoy::PHI),
...
unsigned
GameBoyTargetLowering::getInlineAsmMemConstraint(StringRef ConstraintCode) const {
...
GameBoyTargetLowering::ConstraintWeight
GameBoyTargetLowering::getSingleConstraintMatchWeight(
...
GameBoyTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
  ...
  if (Constraint.size() == 1) {
    ...
    case 'a': // Simple upper registers r16..r23.
      if (VT == MVT::i8)
        return std::make_pair(0U, &GameBoy::LD8loRegClass);
      else if (VT == MVT::i16)
        return std::make_pair(0U, &GameBoy::DREGSLD8loRegClass);
      break;
    case 'b': // Base pointer registers: y, z.
      if (VT == MVT::i8 || VT == MVT::i16)
        return std::make_pair(0U, &GameBoy::PTRDISPREGSRegClass);
      break;
    case 'd': // Upper registers r16..r31.
      if (VT == MVT::i8)
        return std::make_pair(0U, &GameBoy::LD8RegClass);
      else if (VT == MVT::i16)
        return std::make_pair(0U, &GameBoy::DLDREGSRegClass);
      break;
    case 'l': // Lower registers r0..r15.
      if (VT == MVT::i8)
        return std::make_pair(0U, &GameBoy::GPR8loRegClass);
      else if (VT == MVT::i16)
        return std::make_pair(0U, &GameBoy::DREGSloRegClass);
      break;
    case 'e': // Pointer register pairs: x, y, z.
      if (VT == MVT::i8 || VT == MVT::i16)
        return std::make_pair(0U, &GameBoy::PTRREGSRegClass);
      break;
    case 'q': // Stack pointer register: SPH:SPL.
      return std::make_pair(0U, &GameBoy::GPRSPRegClass);
    case 'r': // Any register: r0..r31.
      if (VT == MVT::i8)
        return std::make_pair(0U, &GameBoy::GPR8RegClass);
      else if (VT == MVT::i16)
        return std::make_pair(0U, &GameBoy::DREGSRegClass);
      break;
    case 't': // Temporary register: r0.
      if (VT == MVT::i8)
        return std::make_pair(unsigned(GameBoy::R0), &GameBoy::GPR8RegClass);
      break;
    case 'w': // Special upper register pairs: r24, r26, r28, r30.
      if (VT == MVT::i8 || VT == MVT::i16)
        return std::make_pair(0U, &GameBoy::IWREGSRegClass);
      break;
    case 'x': // Pointer register pair X: r27:r26.
    case 'X':
      if (VT == MVT::i8 || VT == MVT::i16)
        return std::make_pair(unsigned(GameBoy::R27R26), &GameBoy::PTRREGSRegClass);
      break;
    case 'y': // Pointer register pair Y: r29:r28.
    case 'Y':
      if (VT == MVT::i8 || VT == MVT::i16)
        return std::make_pair(unsigned(GameBoy::R29R28), &GameBoy::PTRREGSRegClass);
      break;
    case 'z': // Pointer register pair Z: r31:r30.
    case 'Z':
      if (VT == MVT::i8 || VT == MVT::i16)
        return std::make_pair(unsigned(GameBoy::R31R30), &GameBoy::PTRREGSRegClass);
      break;
    default:
      break;
    }
...
void GameBoyTargetLowering::LowerAsmOperandForConstraint(SDValue Op,
...
Register GameBoyTargetLowering::getRegisterByName(const char *RegName, LLT VT,
