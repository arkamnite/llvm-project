//===- MC/GameBoyMCAsmStreamer.cpp - Text Assembly Output ----------*- C++ -*-===//
//
...
class GameBoyMCAsmStreamer final : public MCStreamer {
...
public:
  GameBoyMCAsmStreamer(MCContext &Context, std::unique_ptr<formatted_raw_ostream> os,
...
void GameBoyMCAsmStreamer::AddComment(const Twine &T, bool EOL) {
...
void GameBoyMCAsmStreamer::EmitCommentsAndEOL() {
...
void GameBoyMCAsmStreamer::emitRawComment(const Twine &T, bool TabPrefix) {
  if (TabPrefix)
    OS << '\t';
...
void GameBoyMCAsmStreamer::addExplicitComment(const Twine &T) {
...
void GameBoyMCAsmStreamer::emitExplicitComments() {
...
void GameBoyMCAsmStreamer::changeSection(MCSection *Section,
...
void GameBoyMCAsmStreamer::emitELFSymverDirective(const MCSymbol *OriginalSym,
                                           StringRef Name,
...
void GameBoyMCAsmStreamer::emitLabel(MCSymbol *Symbol, SMLoc Loc) {
...
void GameBoyMCAsmStreamer::emitLOHDirective(MCLOHType Kind, const MCLOHArgs &Args) {
...
void GameBoyMCAsmStreamer::emitGNUAttribute(unsigned Tag, unsigned Value) {
  OS << "\t.gnu_attribute " << Tag << ", " << Value << "\n";
}

void GameBoyMCAsmStreamer::emitAssemblerFlag(MCAssemblerFlag Flag) {
  switch (Flag) {
...
void GameBoyMCAsmStreamer::emitLinkerOptions(ArrayRef<std::string> Options) {
...
void GameBoyMCAsmStreamer::emitDataRegion(MCDataRegionType Kind) {
...
void GameBoyMCAsmStreamer::emitVersionMin(MCVersionMinType Type, unsigned Major,
                                   unsigned Minor, unsigned Update,
...
void GameBoyMCAsmStreamer::emitBuildVersion(unsigned Platform, unsigned Major,
                                     unsigned Minor, unsigned Update,
                                     VersionTuple SDKVersion) {
...
void GameBoyMCAsmStreamer::emitAssignment(MCSymbol *Symbol, const MCExpr *Value) {
...
void GameBoyMCAsmStreamer::emitConditionalAssignment(MCSymbol *Symbol,
                                              const MCExpr *Value) {
...
void GameBoyMCAsmStreamer::emitWeakReference(MCSymbol *Alias, const MCSymbol *Symbol) {
  OS << ".weakref ";
  Alias->print(OS, MAI);
  OS << ", ";
  Symbol->print(OS, MAI);
...
void GameBoyMCAsmStreamer::emitSymbolDesc(MCSymbol *Symbol, unsigned DescValue) {
  OS << ".desc" << ' ';
  Symbol->print(OS, MAI);
  OS << ',' << DescValue;
  EmitEOL();
...
void GameBoyMCAsmStreamer::emitSyntaxDirective() {
...
void GameBoyMCAsmStreamer::beginCOFFSymbolDef(const MCSymbol *Symbol) {
  OS << "\t.def\t";
  Symbol->print(OS, MAI);
  OS << ';';
  EmitEOL();
...
void GameBoyMCAsmStreamer::emitCOFFSymbolStorageClass(int StorageClass) {
  OS << "\t.scl\t" << StorageClass << ';';
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitCOFFSymbolType(int Type) {
  OS << "\t.type\t" << Type << ';';
  EmitEOL();
}

void GameBoyMCAsmStreamer::endCOFFSymbolDef() {
  OS << "\t.endef";
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitCOFFSafeSEH(MCSymbol const *Symbol) {
  OS << "\t.safeseh\t";
  Symbol->print(OS, MAI);
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitCOFFSymbolIndex(MCSymbol const *Symbol) {
  OS << "\t.symidx\t";
  Symbol->print(OS, MAI);
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitCOFFSectionIndex(MCSymbol const *Symbol) {
  OS << "\t.secidx\t";
  Symbol->print(OS, MAI);
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitCOFFSecRel32(MCSymbol const *Symbol, uint64_t Offset) {
  OS << "\t.secrel32\t";
  Symbol->print(OS, MAI);
  if (Offset != 0)
    OS << '+' << Offset;
...
void GameBoyMCAsmStreamer::emitCOFFImgRel32(MCSymbol const *Symbol, int64_t Offset) {
  OS << "\t.rva\t";
  Symbol->print(OS, MAI);
  if (Offset > 0)
    OS << '+' << Offset;
...
void GameBoyMCAsmStreamer::emitXCOFFLocalCommonSymbol(MCSymbol *LabelSym,
                                               uint64_t Size,
                                               MCSymbol *CsectSym,
...
void GameBoyMCAsmStreamer::emitXCOFFSymbolLinkageWithVisibility(
...
void GameBoyMCAsmStreamer::emitXCOFFRenameDirective(const MCSymbol *Name,
                                             StringRef Rename) {
  OS << "\t.rename\t";
  Name->print(OS, MAI);
  const char DQ = '"';
...
void GameBoyMCAsmStreamer::emitXCOFFRefDirective(StringRef Name) {
  OS << "\t.ref " << Name;
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitELFSize(MCSymbol *Symbol, const MCExpr *Value) {
...
void GameBoyMCAsmStreamer::emitCommonSymbol(MCSymbol *Symbol, uint64_t Size,
                                     unsigned ByteAlignment) {
  OS << "\t.comm\t";
  Symbol->print(OS, MAI);
  OS << ',' << Size;
...
void GameBoyMCAsmStreamer::emitLocalCommonSymbol(MCSymbol *Symbol, uint64_t Size,
                                          unsigned ByteAlign) {
  OS << "\t.lcomm\t";
  Symbol->print(OS, MAI);
  OS << ',' << Size;
...
void GameBoyMCAsmStreamer::emitZerofill(MCSection *Section, MCSymbol *Symbol,
...
// e.g. _a.
void GameBoyMCAsmStreamer::emitTBSSSymbol(MCSection *Section, MCSymbol *Symbol,
                                   uint64_t Size, unsigned ByteAlignment) {
  assignFragment(Symbol, &Section->getDummyFragment());
...
void GameBoyMCAsmStreamer::PrintQuotedString(StringRef Data, raw_ostream &OS) const {
  OS << '"';
...
void GameBoyMCAsmStreamer::emitBytes(StringRef Data) {
...
void GameBoyMCAsmStreamer::emitBinaryData(StringRef Data) {
...
void GameBoyMCAsmStreamer::emitIntValue(uint64_t Value, unsigned Size) {
  emitValue(MCConstantExpr::create(Value, getContext()), Size);
}

void GameBoyMCAsmStreamer::emitIntValueInHex(uint64_t Value, unsigned Size) {
  emitValue(MCConstantExpr::create(Value, getContext(), true), Size);
}

void GameBoyMCAsmStreamer::emitIntValueInHexWithPadding(uint64_t Value,
                                                 unsigned Size) {
  emitValue(MCConstantExpr::create(Value, getContext(), true, Size), Size);
}

void GameBoyMCAsmStreamer::emitValueImpl(const MCExpr *Value, unsigned Size,
                                  SMLoc Loc) {
...
void GameBoyMCAsmStreamer::emitULEB128Value(const MCExpr *Value) {
  int64_t IntValue;
  if (Value->evaluateAsAbsolute(IntValue)) {
...
void GameBoyMCAsmStreamer::emitSLEB128Value(const MCExpr *Value) {
  int64_t IntValue;
  if (Value->evaluateAsAbsolute(IntValue)) {
...
void GameBoyMCAsmStreamer::emitDTPRel64Value(const MCExpr *Value) {
...
void GameBoyMCAsmStreamer::emitDTPRel32Value(const MCExpr *Value) {
...
void GameBoyMCAsmStreamer::emitTPRel64Value(const MCExpr *Value) {
...
void GameBoyMCAsmStreamer::emitTPRel32Value(const MCExpr *Value) {
...
void GameBoyMCAsmStreamer::emitGPRel64Value(const MCExpr *Value) {
...
void GameBoyMCAsmStreamer::emitGPRel32Value(const MCExpr *Value) {
...
void GameBoyMCAsmStreamer::emitFill(const MCExpr &NumBytes, uint64_t FillValue,
                             SMLoc Loc) {
  int64_t IntNumBytes;
...
void GameBoyMCAsmStreamer::emitFill(const MCExpr &NumValues, int64_t Size,
...
void GameBoyMCAsmStreamer::emitValueToAlignment(unsigned ByteAlignment, int64_t Value,
                                         unsigned ValueSize,
...
void GameBoyMCAsmStreamer::emitCodeAlignment(unsigned ByteAlignment,
...
void GameBoyMCAsmStreamer::emitValueToOffset(const MCExpr *Offset,
                                      unsigned char Value,
                                      SMLoc Loc) {
...
void GameBoyMCAsmStreamer::emitFileDirective(StringRef Filename) {
...
void GameBoyMCAsmStreamer::printDwarfFileDirective(
...
Expected<unsigned> GameBoyMCAsmStreamer::tryEmitDwarfFileDirective(
    unsigned FileNo, StringRef Directory, StringRef Filename,
...
void GameBoyMCAsmStreamer::emitDwarfFile0Directive(StringRef Directory,
                                            StringRef Filename,
                                            Optional<MD5::MD5Result> Checksum,
                                            Optional<StringRef> Source,
                                            unsigned CUID) {
...
void GameBoyMCAsmStreamer::emitDwarfLocDirective(unsigned FileNo, unsigned Line,
...
MCSymbol *GameBoyMCAsmStreamer::getDwarfLineTableSymbol(unsigned CUID) {
  // Always use the zeroth line table, since asm syntax only supports one line
  // table for now.
  return MCStreamer::getDwarfLineTableSymbol(0);
}

bool GameBoyMCAsmStreamer::emitCVFileDirective(unsigned FileNo, StringRef Filename,
                                        ArrayRef<uint8_t> Checksum,
...
bool GameBoyMCAsmStreamer::emitCVFuncIdDirective(unsigned FuncId) {
  OS << "\t.cv_func_id " << FuncId << '\n';
  return MCStreamer::emitCVFuncIdDirective(FuncId);
}

bool GameBoyMCAsmStreamer::emitCVInlineSiteIdDirective(unsigned FunctionId,
                                                unsigned IAFunc,
                                                unsigned IAFile,
...
void GameBoyMCAsmStreamer::emitCVLocDirective(unsigned FunctionId, unsigned FileNo,
...
void GameBoyMCAsmStreamer::emitCVLinetableDirective(unsigned FunctionId,
...
void GameBoyMCAsmStreamer::emitCVInlineLinetableDirective(unsigned PrimaryFunctionId,
                                                   unsigned SourceFileId,
                                                   unsigned SourceLineNum,
                                                   const MCSymbol *FnStartSym,
...
void GameBoyMCAsmStreamer::PrintCVDefRangePrefix(
    ArrayRef<std::pair<const MCSymbol *, const MCSymbol *>> Ranges) {
  OS << "\t.cv_def_range\t";
...
void GameBoyMCAsmStreamer::emitCVDefRangeDirective(
    ArrayRef<std::pair<const MCSymbol *, const MCSymbol *>> Ranges,
...
void GameBoyMCAsmStreamer::emitCVStringTableDirective() {
  OS << "\t.cv_stringtable";
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitCVFileChecksumsDirective() {
  OS << "\t.cv_filechecksums";
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitCVFileChecksumOffsetDirective(unsigned FileNo) {
  OS << "\t.cv_filechecksumoffset\t" << FileNo;
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitCVFPOData(const MCSymbol *ProcSym, SMLoc L) {
  OS << "\t.cv_fpo_data\t";
  ProcSym->print(OS, MAI);
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitIdent(StringRef IdentString) {
...
void GameBoyMCAsmStreamer::emitCFISections(bool EH, bool Debug) {
...
void GameBoyMCAsmStreamer::emitCFIStartProcImpl(MCDwarfFrameInfo &Frame) {
  OS << "\t.cfi_startproc";
  if (Frame.IsSimple)
    OS << " simple";
  EmitEOL();
...
void GameBoyMCAsmStreamer::emitCFIEndProcImpl(MCDwarfFrameInfo &Frame) {
  MCStreamer::emitCFIEndProcImpl(Frame);
  OS << "\t.cfi_endproc";
  EmitEOL();
}

void GameBoyMCAsmStreamer::EmitRegisterName(int64_t Register) {
...
void GameBoyMCAsmStreamer::emitCFIDefCfa(int64_t Register, int64_t Offset) {
...
void GameBoyMCAsmStreamer::emitCFIDefCfaOffset(int64_t Offset) {
  MCStreamer::emitCFIDefCfaOffset(Offset);
  OS << "\t.cfi_def_cfa_offset " << Offset;
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitCFILLVMDefAspaceCfa(int64_t Register, int64_t Offset,
...
void GameBoyMCAsmStreamer::emitCFIEscape(StringRef Values) {
  MCStreamer::emitCFIEscape(Values);
  PrintCFIEscape(OS, Values);
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitCFIGnuArgsSize(int64_t Size) {
...
void GameBoyMCAsmStreamer::emitCFIDefCfaRegister(int64_t Register) {
...
void GameBoyMCAsmStreamer::emitCFIOffset(int64_t Register, int64_t Offset) {
...
void GameBoyMCAsmStreamer::emitCFIPersonality(const MCSymbol *Sym,
                                       unsigned Encoding) {
...
void GameBoyMCAsmStreamer::emitCFILsda(const MCSymbol *Sym, unsigned Encoding) {
...
void GameBoyMCAsmStreamer::emitCFIRememberState() {
  MCStreamer::emitCFIRememberState();
  OS << "\t.cfi_remember_state";
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitCFIRestoreState() {
  MCStreamer::emitCFIRestoreState();
  OS << "\t.cfi_restore_state";
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitCFIRestore(int64_t Register) {
...
void GameBoyMCAsmStreamer::emitCFISameValue(int64_t Register) {
...
void GameBoyMCAsmStreamer::emitCFIRelOffset(int64_t Register, int64_t Offset) {
...
void GameBoyMCAsmStreamer::emitCFIAdjustCfaOffset(int64_t Adjustment) {
  MCStreamer::emitCFIAdjustCfaOffset(Adjustment);
  OS << "\t.cfi_adjust_cfa_offset " << Adjustment;
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitCFISignalFrame() {
  MCStreamer::emitCFISignalFrame();
  OS << "\t.cfi_signal_frame";
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitCFIUndefined(int64_t Register) {
...
void GameBoyMCAsmStreamer::emitCFIRegister(int64_t Register1, int64_t Register2) {
...
void GameBoyMCAsmStreamer::emitCFIWindowSave() {
  MCStreamer::emitCFIWindowSave();
  OS << "\t.cfi_window_save";
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitCFINegateRAState() {
  MCStreamer::emitCFINegateRAState();
  OS << "\t.cfi_negate_ra_state";
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitCFIReturnColumn(int64_t Register) {
...
void GameBoyMCAsmStreamer::emitCFIBKeyFrame() {
  MCStreamer::emitCFIBKeyFrame();
  OS << "\t.cfi_b_key_frame";
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitCFIMTETaggedFrame() {
  MCStreamer::emitCFIMTETaggedFrame();
  OS << "\t.cfi_mte_tagged_frame";
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitWinCFIStartProc(const MCSymbol *Symbol, SMLoc Loc) {
...
void GameBoyMCAsmStreamer::emitWinCFIEndProc(SMLoc Loc) {
...
void GameBoyMCAsmStreamer::emitWinCFIFuncletOrFuncEnd(SMLoc Loc) {
...
void GameBoyMCAsmStreamer::emitWinCFIStartChained(SMLoc Loc) {
...
void GameBoyMCAsmStreamer::emitWinCFIEndChained(SMLoc Loc) {
...
void GameBoyMCAsmStreamer::emitWinEHHandler(const MCSymbol *Sym, bool Unwind,
...
void GameBoyMCAsmStreamer::emitWinEHHandlerData(SMLoc Loc) {
...
void GameBoyMCAsmStreamer::emitWinCFIPushReg(MCRegister Register, SMLoc Loc) {
...
void GameBoyMCAsmStreamer::emitWinCFISetFrame(MCRegister Register, unsigned Offset,
                                       SMLoc Loc) {
...
void GameBoyMCAsmStreamer::emitWinCFIAllocStack(unsigned Size, SMLoc Loc) {
...
void GameBoyMCAsmStreamer::emitWinCFISaveReg(MCRegister Register, unsigned Offset,
                                      SMLoc Loc) {
...
void GameBoyMCAsmStreamer::emitWinCFISaveXMM(MCRegister Register, unsigned Offset,
                                      SMLoc Loc) {
...
void GameBoyMCAsmStreamer::emitWinCFIPushFrame(bool Code, SMLoc Loc) {
...
void GameBoyMCAsmStreamer::emitWinCFIEndProlog(SMLoc Loc) {
...
void GameBoyMCAsmStreamer::emitCGProfileEntry(const MCSymbolRefExpr *From,
...
void GameBoyMCAsmStreamer::AddEncodingComment(const MCInst &Inst,
                                       const MCSubtargetInfo &STI) {
...
void GameBoyMCAsmStreamer::emitInstruction(const MCInst &Inst,
                                    const MCSubtargetInfo &STI) {
  assert(getCurrentSectionOnly() &&
         "Cannot emit contents before setting section!");
...
void GameBoyMCAsmStreamer::emitPseudoProbe(
...
void GameBoyMCAsmStreamer::emitBundleAlignMode(unsigned AlignPow2) {
  OS << "\t.bundle_align_mode " << AlignPow2;
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitBundleLock(bool AlignToEnd) {
  OS << "\t.bundle_lock";
  if (AlignToEnd)
    OS << " align_to_end";
  EmitEOL();
...
void GameBoyMCAsmStreamer::emitBundleUnlock() {
  OS << "\t.bundle_unlock";
  EmitEOL();
}

Optional<std::pair<bool, std::string>>
GameBoyMCAsmStreamer::emitRelocDirective(const MCExpr &Offset, StringRef Name,
...
void GameBoyMCAsmStreamer::emitAddrsig() {
  OS << "\t.addrsig";
  EmitEOL();
}

void GameBoyMCAsmStreamer::emitAddrsigSym(const MCSymbol *Sym) {
  OS << "\t.addrsig_sym ";
  Sym->print(OS, MAI);
  EmitEOL();
}
...
void GameBoyMCAsmStreamer::emitRawTextImpl(StringRef String) {
...
void GameBoyMCAsmStreamer::finishImpl() {
...
void GameBoyMCAsmStreamer::emitDwarfUnitLength(uint64_t Length, const Twine &Comment) {
...
MCSymbol *GameBoyMCAsmStreamer::emitDwarfUnitLength(const Twine &Prefix,
                                             const Twine &Comment) {
  // If the assembler on some target fills in the DWARF unit length, we
  // don't want to emit the length in the compiler. For example, the AIX
  // assembler requires the assembly file with the unit length omitted from
...
void GameBoyMCAsmStreamer::emitDwarfLineStartLabel(MCSymbol *StartSym) {
  // If the assembler on some target fills in the DWARF unit length, we
  // don't want to emit the length in the compiler. For example, the AIX
  // assembler requires the assembly file with the unit length omitted from
  // the debug section headers. In such cases, any label we placed occurs
...
void GameBoyMCAsmStreamer::emitDwarfLineEndEntry(MCSection *Section,
                                          MCSymbol *LastLabel) {
...
void GameBoyMCAsmStreamer::emitDwarfAdvanceLineAddr(int64_t LineDelta,
...
void GameBoyMCAsmStreamer::doFinalizationAtSectionEnd(MCSection *Section) {
...
MCStreamer *llvm::createAsmStreamer(MCContext &Context,
                                    ...
                                    bool ShowInst) {
  return new GameBoyMCAsmStreamer(Context, std::move(OS), isVerboseAsm,
